# -*- coding: utf-8 -*-
"""Python e GTK: PyGObject libadwaita style class body."""
...
class ExampleWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        self.set_title(title='Python e GTK: PyGObject style classe body')
        ...
        label = Gtk.Label.new(
            str='A classe de estilo .body é o estilo de texto padrão.',
        )
    ...
    def on_button_clicked(self, button):
        if 'body' in self.label_body.get_css_classes():
            self.label_body.remove_css_class(css_class='body')
        else:
            self.label_body.add_css_class(css_class='body')
        self.label_body.set_text(
            str=LOREM.format(f'Classes: {self.label_body.get_css_classes()}'),
        )
