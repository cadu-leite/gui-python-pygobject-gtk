# -*- coding: utf-8 -*-
"""Python e GTK: PyGObject libadwaita style class lowres-icon."""
...
class ExampleWindow(Gtk.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        self.set_title(
            title='Python e GTK: PyGObject libadwaita style class lowres-icon')
        ...
        self.label = Gtk.Label.new(str=TEXT.format("Classes: ['lowres-icon']"))
        self.label.set_vexpand(expand=True)
        self.label.set_wrap(wrap=True)
        vbox.append(child=self.label)

        self.image_lowres_icon = Gtk.Image.new_from_icon_name(
            icon_name='software-update-urgent-symbolic',
        )
        self.image_lowres_icon.set_pixel_size(pixel_size=32)
        self.image_lowres_icon.add_css_class(css_class='lowres-icon')
        vbox.append(child=self.image_lowres_icon)
    ...
    def on_button_clicked(self, button):
        if 'lowres-icon' in self.image_lowres_icon.get_css_classes():
            self.image_lowres_icon.remove_css_class(css_class='lowres-icon')
        else:
            self.image_lowres_icon.add_css_class(css_class='lowres-icon')
        self.label.set_text(
            str=TEXT.format(self.image_lowres_icon.get_css_classes()),
        )
