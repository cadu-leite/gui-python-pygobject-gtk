# -*- coding: utf-8 -*-
"""Python e GTK: PyGObject libadwaita style class background ui file."""
...
class ExampleWindow(Adw.ApplicationWindow):
    ...
    button_background = Gtk.Template.Child(name='button_background')
    ...
    def on_button_clicked(self, button):
        if 'background' in self.button_background.get_css_classes():
            self.button_background.remove_css_class(css_class='background')
        else:
            self.button_background.add_css_class(css_class='background')
        self.button_background.set_label(
            label=f'Classes: {self.button_background.get_css_classes()}',
        )
